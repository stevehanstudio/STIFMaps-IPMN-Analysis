...
def resize_crop_image(input_filename, base_name, orig_dapi):
    ...
    # stitched_image = tifffile.imread(input_filename)

    # Open the stitched image (handle PNG files with Pillow)
    if input_filename.lower().endswith('.png'):
        stitched_image = np.array(Image.open(input_filename))
    else:
        stitched_image = tifffile.imread(input_filename)
    ...
    resized_image = resize(stitched_image, (total_height, total_width), anti_aliasing=True)
    ...
    resized_image_filename = os.path.join('/home/steve/Projects/WeaverLab/STIFMaps', f"{base_name}_resized.tif")
    tifffile.imwrite(resized_image_filename, resized_image.astype(np.uint16))
    resized_width, resized_height = check_image_dimensions(resized_image_filename)
    print(f"Resized image saved as {resized_image_filename} at {resized_width}x{resized_height}")
    ...
    output_filename = os.path.join('/home/steve/Projects/WeaverLab/STIFMaps', f"{base_name}_cropped.tif")
    tifffile.imwrite(output_filename, cropped_image.astype(np.uint16))
